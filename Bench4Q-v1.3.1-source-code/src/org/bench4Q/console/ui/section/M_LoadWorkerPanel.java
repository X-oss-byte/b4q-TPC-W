 ...
 * 
 */
public class M_LoadWorkerPanel extends JPanel implements ActionListener {
	 ...
	 * 
	 */
	public M_LoadWorkerPanel(Resources resources, ConfigModel configModel) {
		...
		JComboBox MixCombo = new JComboBox();
		MixCombo.addItem("Ordering");
		MixCombo.addItem("Shopping");
		MixCombo.addItem("Browsing");
		TableCellEditor mixEditor = new DefaultCellEditor(MixCombo);
		TableColumnModel columnModel = table.getColumnModel();
		TableColumn mixColumn = columnModel.getColumn(1);
		mixColumn.setCellEditor(mixEditor);
	...
	private class MyTableModel extends AbstractTableModel {
		...
		public Object getValueAt(int rowIndex, int columnIndex) {
			if (columnIndex == 0) {
				...
				return ((TestPhase) data.get(rowIndex)).getMix();
			} else if (columnIndex == 2) {
				return ((TestPhase) data.get(rowIndex)).getThinktime();
		...
		public void setValueAt(Object value, int row, int col) {
			if (col == 0) {
				...
				((TestPhase) data.get(row)).setMix((String) value);
			} else if (col == 2) {
				((TestPhase) data.get(row)).setThinktime(Double
						.valueOf((String) value));
			} else if (col == 3) {
