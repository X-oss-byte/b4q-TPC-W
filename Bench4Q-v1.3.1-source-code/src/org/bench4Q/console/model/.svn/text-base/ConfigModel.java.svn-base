...
public final class ConfigModel {
	...
	public void initFile(File XMLFile) {
		...
		if (m_args.getEbs().isEmpty()) {
			...
			Element Mix = new Element("Mix").setText("Shopping");
			ebs.addContent(Mix);

			Element ThinkTime = new Element("rate").setText("0");
			ebs.addContent(ThinkTime);
			...
			for (TestPhase testPhase : m_args.getEbs()) {
				...
				ebs.addContent(baseLoad);
				Element randomLoad = new Element("Mix").setText(String
						.valueOf(testPhase.getMix()));
				ebs.addContent(randomLoad);
				Element rate = new Element("ThinkTime").setText(String
						.valueOf(testPhase.getThinktime()));
				ebs.addContent(rate);
	...
	public void SaveToFile(File XMLFile) {
		...
		for (TestPhase testPhase : m_args.getEbs()) {
			...
			Element randomLoad = new Element("Mix").setText(String
					.valueOf(testPhase.getMix()));
			ebs.addContent(randomLoad);
			Element rate = new Element("ThinkTime").setText(String.valueOf(testPhase
					.getThinktime()));
			ebs.addContent(rate);
			Element triggerTime = new Element("triggerTime").setText(String
					.valueOf(testPhase.getTriggerTime()));
	...
	private void initRBE() {
		...
		for (int j = 0; j < ebs.size(); j++) {
			...
			testPhase.setMix(eb.getChildText("Mix").trim());
			testPhase.setThinktime(Integer.parseInt(eb.getChildText("ThinkTime")));
